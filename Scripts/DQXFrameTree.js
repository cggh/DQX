...
DQX.Framework.TreeCtrl = {};

////////////////////////// A class representing a branch in the tree ///////////////////////////////////////

DQX.Framework.TreeCtrl._objectBranch = function (iID, icontent) {
    this.myID = iID;
...
DQX.Framework.TreeCtrl._objectBranch.prototype.renderHtml = function () {
    return this.content.toString();
}

DQX.Framework.TreeCtrl._objectBranch.prototype.postCreateHtml = function () {
}


DQX.Framework.TreeCtrl._objectBranch.prototype.getItems = function () {
    return this.items;
}

DQX.Framework.TreeCtrl._objectBranch.prototype.addItem = function (item) {
...
DQX.Framework.TreeCtrl._objectBranch.prototype.setTree = function (tree) {
...
DQX.Framework.TreeCtrl._objectBranch.prototype._setTreeSub = function (tree) { }


//The creator function
DQX.Framework.TreeCtrl.Branch = function (iID,icontent) {
    return new DQX.Framework.TreeCtrl._objectBranch(iID,icontent);
}


//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

DQX.Framework.TreeCtrl.Control = function (icontrol) {
    var that = DQX.Framework.TreeCtrl.Branch(icontrol.getID(), '');
...
DQX.Framework.TreeCtrl.Tree = function (iid, idivid) {
    var that = {};
    that.myID = iid;
    that.myDivID = idivid;
    that.root = DQX.Framework.TreeCtrl.Branch(); //tree structured container of the items
    ...
    that._renderSub = function (parentDiv, item, level) {
        ...
        var titledv = DQX.DocEl.Div({ parent: parentDiv, id: this._getDivIDItem(item.myID) });
        if (item.canSelect)
        ...
        if (hasSubItems) {
            var buttondv = DQX.DocEl.Div({ parent: titledv, id: this._getDivIDButton(item.myID) });
        ...
        if (hasSubItems) {
            var descdv = DQX.DocEl.Div({ parent: parentDiv, id: this.myID + '_DQXSub_' + item.myID });
            descdv.addStyle('padding-left', '12px');
            descdv.addStyle('padding-bottom', '4px');
            var descdv2 = DQX.DocEl.Div({ parent: descdv });
