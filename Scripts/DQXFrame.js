...
window.onhashchange = $.proxy(DQX.HistoryManager.onChanged, DQX.HistoryManager);
...
DQX.Msg._listeners = [];
DQX.Msg._listeneridmap = {};

DQX.Msg.send = function (scope, content) {
    for (var lnr = 0; lnr < DQX.Msg._listeners.length; lnr++) {
        var lscope = DQX.Msg._listeners[lnr].scopeFilter;
        var covered = true;
        ...
        if (covered)
            DQX.Msg._listeners[lnr].callbackFunction.call(DQX.Msg._listeners[lnr].context, scope, content);
    }
}

DQX.Msg.listen = function (eventid, scopeFilter, callbackFunction, context) {
    if (typeof (eventid) != 'string')
        throw ('Listener event id not provided');
    if ((eventid != '') && (eventid in DQX.Msg._listeneridmap)) {
        var idx = DQX.Msg._listeneridmap[eventid];
        DQX.Msg._listeners[idx].scopeFilter = scopeFilter;
        DQX.Msg._listeners[idx].callbackFunction = callbackFunction;
        DQX.Msg._listeners[idx].context = context;
        return;
    }
    if (eventid != '')
        DQX.Msg._listeneridmap[eventid] = DQX.Msg._listeners.length;
    DQX.Msg._listeners.push({ eventid: eventid, scopeFilter: scopeFilter, callbackFunction: callbackFunction, context: context });
}
...
DQX.Framework.frameTitleBarH = 33;
DQX.Framework.sepSizeLarge = 14;
DQX.Framework.sepSizeSmall = 6;


DQX.Framework.FrameTypes = [
...
DQX.Framework.SizeRange = function () {
    var that = {};
    that.minSize = 120;
...
DQX.Framework.FrameGroupHor = function (iid, isizeweight) {
    return DQX.Framework.Frame(iid, 'GroupHor', isizeweight);
}

DQX.Framework.FrameGroupVert = function (iid, isizeweight) {
    return DQX.Framework.Frame(iid, 'GroupVert', isizeweight);
}

DQX.Framework.FrameGroupTab = function (iid, isizeweight) {
    return DQX.Framework.Frame(iid, 'Tab', isizeweight);
}

DQX.Framework.FrameFinal = function (iid, isizeweight) {
    return DQX.Framework.Frame(iid, 'Final', isizeweight);
}


DQX.Framework.Frame = function (iid, itype, isizeweight) {
    var that = {};
    ...
    that.sizeRange = [DQX.Framework.SizeRange(), DQX.Framework.SizeRange()]; //allowed frame size range in X and Y dir
    that.marginLeft = 0;
    that.marginRight = 0;
    that.marginTop = 0;
    that.marginBottom = 0;
    ...
    that.setDisplayTitle = function (ttle) {
        this.marginTop = DQX.Framework.frameTitleBarH;
    ...
    that.getMinSize = function (dim) {
        ...
        if ((dim == DQX.dimY) && this.myDisplayTitle)
            minsize += DQX.Framework.frameTitleBarH;
        return minsize;
    }
    ...
    that.getMaxSize = function (dim) {
        ...
        if ((dim == DQX.dimY) && this.myDisplayTitle)
            maxsize += DQX.Framework.frameTitleBarH;
        return maxsize;
    }
    ...
    that.createElements = function (level) {
        ...
        var thediv = DQX.DocEl.Div({ id: this.myID });
        var theclientdiv = DQX.DocEl.Div({ parent: thediv, id: this.getClientDivID() });
        ...
        if (this.myDisplayTitle) {
            var titlediv = DQX.DocEl.Div({ parent: thediv, id: this.getVisibleTyleDivID() });
            titlediv.setHeightPx(DQX.Framework.frameTitleBarH);
        ...
        if (this.isSplitter()) {
            for (var fnr = 0; fnr < this.memberFrames.length - 1; fnr++) {
                var splitdiv = DQX.DocEl.Div({ parent: thediv, id: this.getSeparatorDivID(fnr) });
        ...
        if (this.isTabber()) {
            ...
            var tabheader = DQX.DocEl.Div({ parent: theclientdiv, id: this.getClientDivID() + '_tabheader' });
            tabheader.setCssClass("DQXTabs");

            var tabbody = DQX.DocEl.Div({ parent: theclientdiv, id: this.getClientDivID() + '_tabbody' });
            tabbody.setCssClass("DQXTabBody");
            this.activeTabNr = 0;

            for (var fnr = 0; fnr < this.memberFrames.length; fnr++) {
                var tabid = this.getClientDivID() + '_tab_' + fnr;
                var tabheaderitem = DQX.DocEl.Div({ parent: tabheader, id: tabid });
                tabheaderitem.setCssClass(fnr == 0 ? "DQXTab DQXTabActive" : "DQXTab DQXTabInactive");
                tabheaderitem.addElem(this.memberFrames[fnr].myFrameID);

                var tabcontent = DQX.DocEl.Div({ parent: tabbody, id: 'C' + tabid });
                tabcontent.setCssClass("DQXTabContent");
                tabcontent.addElem(this.memberFrames[fnr].createElements(level + 1));
            }

            DQX.Msg.listen('',{ type: 'ClickTab', id: this.getClientDivID() }, that._reactClickTab, that);
    ...
    that.onChangeTab = function (newtab) {
        DQX.Msg.send({ type: 'ChangeTab', id: this.myFrameID }, this.getActiveTabFrameID());
    }
...
DQX.Framework._handleResize = function () {
    var myparent = $('#' + DQX.Framework.frameRoot.myID).parent();
    var v1 = myparent.attr('id');
    var v2 = myparent.get(0).tagName;
    var sx = myparent.innerWidth();
    var sy = myparent.innerHeight();
    DQX.Framework.frameRoot.setPosition(0, 0, sx, sy);
}
...
DQX.Framework.render = function (frameRoot, divid) {
    DQX.Framework.frameRoot = frameRoot;
    var html = frameRoot.createElements(1).toString();
    $('#' + divid).html(html);
    frameRoot.postCreateHTML();

    $(window).resize(DQX.Framework._handleResize)

    DQX.Framework._handleResize();
}
...
DQX.Framework.Form = function (iid, idivid) {
    var that = {};
    that.myID = iid;
    that.myDivID = idivid;
    that._content = DQX.Controls.CompoundHor([]);
    ...
    that.addHtml = function (content) {
        that._content.append(DQX.Controls.Label(content));
    }
