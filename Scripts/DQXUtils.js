...
DQX.initPostCreate = function () {
    ...
    $('.DQXTabSet').each(function (idx, tabset) {
        ...
        $(tabset).children('.DQXTabs').children('.DQXTab').click(function () {
            ...
            DQX.Msg.send({ type: 'ClickTab', id: id }, this.id);
...
function randomWord(startCapital) {
    var rs = '';
    var len = 3 + DQX.random() * 5;
    for (var i = 0; i < len; i++)
        if (startCapital && (i == 0))
            rs += alphabetcap.charAt(DQX.random() * 26);
        else
            rs += alphabet.charAt(DQX.random() * 26);
    return rs;
}

function randomSentence(maxlen) {
    var rs = randomWord(true);
    var len = 1 + DQX.random() * (maxlen - 2);
    for (var i = 0; i < len; i++)
        rs += ' ' + randomWord(false);
    rs += '.';
    return rs;
...
function randomText() {
    var rs = '';
    var len = 9 + DQX.random() * 12;
    for (var i = 0; i < len; i++) {
        if (i > 0) rs += ' ';
        rs += randomSentence(15);
        if (DQX.random() < 0.1) rs += '<p>';
    }
    return rs;
}
