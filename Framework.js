...
/*            if (that.isFinalPanel()) {
        ...
        ////////////////////////////////////////////////////////////////////////////////////////
        //Frame that holds a canvas
        Framework.Canvas = function(iParentRef, template) {
            var that = FramePanel(iParentRef);
            that._panelfirstRendered = false;
            ...
            that.render = function () {
                var ctx = that.getMyCanvasElement().getContext("2d");
            ...
            that.getEventPosX = function (ev) {
                ...
                return ev1.pageX - $(that.getMyCanvasElement()).offset().left;
            }
            ...
            that.getEventPosY = function (ev) {
                ...
                if (ev.originalEvent)
                    ev1 = ev.originalEvent;
                return ev1.pageY - $(that.getMyCanvasElement()).offset().top;
            }

            that.posXCanvas2Screen = function (px) {
                return px + $(that.getMyCanvasElement()).offset().left;
            }

            that.posYCanvas2Screen = function (py) {
                return py + $(that.getMyCanvasElement()).offset().top;
            }
            ...
            that._onMouseDown = function(ev) {
                that.onMouseDown(ev, { x: that.getEventPosX(ev), y: that.getEventPosY(ev), pageY: ev.pageY });
            };

            that._onMouseMove = function(ev) {
                var px = that.getEventPosX(ev);
                var py = that.getEventPosY(ev);
                //that.onHoverOverChannel(px, py);
            ...
            // Initialise
            that.canvasID = that.getDivID()+'_canvas';

            var cnv = DocEl.Create('canvas', { id: that.canvasID });
            that._cnvWidth = $('#' + that.getDivID()).innerWidth();
            that._cnvHeight = $('#' + that.getDivID()).innerHeight();
            ...
            $('#' + that.getDivID()).html(cnv.toString());

            $('#' + that.canvasID).click($.proxy(that._onMouseClick, that));
            $('#' + that.canvasID).mousedown($.proxy(that._onMouseDown, that));
            $('#' + that.canvasID).mousemove($.proxy(that._onMouseMove, that));
            $('#' + that.canvasID).mouseenter($.proxy(that._onMouseEnter, that));
            $('#' + that.canvasID).mouseleave($.proxy(that._onMouseLeave, that));
