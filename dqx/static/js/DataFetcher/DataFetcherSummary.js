define(["DQX/DataDecoders"],
    function(DataDecoders) {
        var Encoder={};
        
        var b64codec = DataDecoders.B64();
        
        Encoder.FixedString = function (info) {
            var that = {};
        ...
        Encoder.Float2B64 = function (info) {
            var that = {};
            ...
            that.decodeArray = function (datastr) {
                return b64codec.arrayB642Float(datastr, this.length, this.slope, this.offset);
            }
        
            return that;
        }

        Encoder.FloatList2B64 = function (info) {
            var that = {};
            ...
            that.decodeArray = function (datastr) {
                ...
                for (var offset = 0; offset < strlen; offset += reclen) {
                    var minval = b64codec.B642IntFixed(datastr, offset + 0, 2) * that.rangeSlope + that.rangeOffset;
                    var maxval = b64codec.B642IntFixed(datastr, offset + 2, 2) * that.rangeSlope + that.rangeOffset;
                    var subrs = [];
                    for (var i = 0; i < this.count; i++)
                        subrs.push(minval + b64codec.B642IntFixed(datastr, offset + 4 + i, 1) / 63.0 * (maxval - minval));
                    rs.push(subrs);
                }
                return rs;
            }
        ...
        Encoder.FixedLengthB64 = function(info) {
            var that = {};
            var scaling_factor = (Math.pow(64,info.length) - 10)/(info.range[1]-info.range[0]);
            that.decodeArray = function(str) {
                out = []
                for (var i = 0; i < str.length; i += info.length) {
                    var s = str.substr(i, info.length);
                    if (s[0] == '~')
                        out.push(null);
                    else
                        out.push(b64codec.B642Int(s)/scaling_factor);
                }
                return out;
            }
            return that;
        ...
        Encoder.Create = function (info) {
//            if (info['ID'] == 'Float2B64')
//                return Encoder.Float2B64(info);
//            if (info['ID'] == 'FloatList2B64')
//                return Encoder.FloatList2B64(info);
//            if (info['ID'] == 'FixedString')
//                return Encoder.FixedString(info);
            //BEN TODO PROPERLY
            return Encoder.FixedLengthB64(info.FixedLengthB64);
        ...
        DataFetcherSummary.SummaryBlockLevel = function (iblocksize) {
            ...
            that._fetchRange = function (keylist) {
                ...
                for (prop in propresults) {
                    ...
                    //TODO Fix
                    var data = dt.data
                    if (dt.accumulators.length == 3) {
                        var d = '';
                        for (var i = 0; i < data.length; i += 9) {
                            d = d + data.substr(i+3, 3);
                        }
                        data = d;
                    }
                    this._buffer[prop] = Encoder.Create(dt.encoder).decodeArray(data);
                }
            }
        ...
        //////////////////////////////////////////////////////////////////////////////////////
        //  Class DataFetcherSummary.Summary
        //////////////////////////////////////////////////////////////////////////////////////
        
        DataFetcherSummary.Summary = function (iserverurl, iminBlockSize, idesiredPointCount) {
            ...
            this._levelBuffers = []; //holds DataFetcherSummary.SummaryBlockLevel
        
            //adds a column to be fetched, providing a column id and a color
            this.addFetchColumn = function (itrackid, color) {
                var cid = itrackid;
                if (cid in this.myColumns)
                    throw "Column id already present: " + cid;
                this.myColumns[cid] = DataFetcherSummary.SummaryColumn(cid, color);
                this.clearData();
            ...
            this._fetchRange = function (blockSize, rangemin, rangemax, needtotalrecordcount) {
                ...
                if (!this._isFetching) {
                    ...
                    myurl.addUrlQueryItem("chromid", this.myChromoID+'_v3');
